...
const cardsOnPage = 5;
const BASE_URL = 'https://practicetasks.kz/prosmotr-mira';
const endpoint = `${BASE_URL}/api/videos?pagination%5BpageSize%5D=${cardsOnPage}&`;
...
function generateFilterRequest(endpoint, city, timeArray) {
  console.log(endpoint)
  if (city) {
    endpoint += `filters%5Bcity%5D%5B$containsi%5D=${city}&`;
  }
  if (timeArray) {
    timeArray.forEach((timeslot) => {
      endpoint += `filters%5Btime_of_day%5D%5B$eqi%5D=${timeslot}&`;
    });
  }
...
function showMoreCards({
  ...
  buttonInDOM.addEventListener('click', async () => {
    ...
    let urlToFetch = `${initialEndpoint}pagination%5Bpage%5D=${(currentPage += 1)}&`;
    try {
